...
const ProductsSoldPerYear = ({ ProductsSoldPerYear }) => {
    const labels = [];
    const graphData = [];

    for (let key in ProductsSoldPerYear) {
        labels.push(key)
        graphData.push(ProductsSoldPerYear[key])
    ...
    const options = {
        ...
        plugins: {
            title: {
                display: true,
                text: 'Products Sold per Year'  // This will be your chart title
            }
        },
        responsive: true,
    ...
    return (
        <div style={{ overflowX: 'auto', width: '40%' }}>
            <div style={{ width: `${totalWidth}px`, minWidth: "470px", height: '300px' }}>
...
export default ProductsSoldPerYear;
